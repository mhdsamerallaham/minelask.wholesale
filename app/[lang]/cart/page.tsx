...
export default function CartPage({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const whatsappUrl = `https://wa.me/${result.admin_phone || ""}?text=${encodeURIComponent(result.message)}`;
    ...
    if (orderComplete) {
        const whatsappUrl = `https://wa.me/${orderComplete.admin_phone || ""}?text=${encodeURIComponent(orderComplete.message)}`;
            ...
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
                <div className="bg-white p-12 rounded-3xl shadow-xl max-w-lg w-full text-center space-y-8">
                    ...
                    <div className="space-y-3 pt-4">
                        <a
                            href={whatsappUrl}
                            className="flex items-center justify-center gap-3 w-full py-4 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-600/20"
                        >
                            ...
                            {lang === 'tr' ? 'WhatsApp ile Devam Et' : lang === 'ar' ? 'المتابعة عبر واتساب' : 'Continue to WhatsApp'}
                        </a>

                        <Link
                            href={`/${lang}/shop`}
                            className="block w-full py-4 bg-slate-100 text-slate-600 rounded-2xl font-bold hover:bg-slate-200 transition-colors"
                        >
